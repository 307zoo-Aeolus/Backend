...
cursor = db.cursor()
sql = "INSERT INTO data_intern('job','salary','city', 'frequency','duration', 'company_name',  'job_link') values(%s,%s,%s,%s,%s,%s,%s)"
...
def parse_html(response,number):
    ...
    try:
       ...
       # for i in soup.find_all('div',class_="f-l intern-detail__job"):
        #    print(i)
...
def save_to_MYSQL(new_data):
    try:
        cursor.execute(sql, (new_data['id'],new_data['city'],new_data['company_name'],new_data['duration'],new_data['frequency'],new_data['job'],new_data['salary'],new_data['link']))
        db.commit()
        print('插入数据成功')
...
if __name__=='__main__':    
    ...
    url='https://www.shixiseng.com/interns?page={}&keyword=&type=intern&area=&months=&days=&degree=&official=&enterprise=&salary=-0&publishTime=&sortType=&city=%E5%85%A8%E5%9B%BD&internExtend='
    ...
    for i in range(0,50):
        ...
        newdata = parse_html(after_decrypt_response,i)
        data.append(newdata)
        for item in newdata:
            save_to_MYSQL(item)
        time.sleep(10*np.random.rand())
        
    with open('internship.json','a+',encoding='utf-8') as f:
        for item in data:
